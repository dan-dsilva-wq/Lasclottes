...
const {
    ...
    failEmailDelivery,
    prepareEmailDeliveryRetry
} = require('./database');

const EMAIL_DELIVERY_KINDS = new Set([
    'guest_payment_confirmation',
    'owner_booking_notification',
    'monitor_booking_notification',
    'owner_checkout_started',
    'monitor_checkout_started'
]);
...
const copyForLanguage = (language) => {
    if (language === 'fr') {
        return {
            ...
            tax: 'Taxe de séjour séparée', taxPending: 'À confirmer selon le classement et le tarif officiel',
            message: 'Demandes particulières', contact: 'Vos coordonnées',
    ...
    if (language === 'nl') {
        return {
            ...
            tax: 'Afzonderlijke toeristenbelasting', taxPending: 'Wordt bevestigd op basis van de classificatie en het officiële tarief',
            message: 'Speciale verzoeken', contact: 'Uw contactgegevens',
    ...
    return {
        ...
        tax: 'Separate tourist tax', taxPending: 'To be confirmed from the classification and official rate',
        message: 'Special requests', contact: 'Your contact details',
...
const agreementSnapshot = (booking) => {
...
const bookingDetails = (booking, copy) => {
    ...
    const snapshot = agreementSnapshot(booking);
    const touristTax = snapshot.price?.touristTaxStatus === 'pending_owner_confirmation'
        ? copy.taxPending
        : money(booking.tourist_tax_eur_cents, 'EUR');
    const rows = [
...
const buildCheckoutStartedEmail = (booking) => {
    const copy = copyForLanguage('en');
    const customerName = `${booking.first_name} ${booking.last_name}`.trim();
    const { rows } = bookingDetails(booking, copy);
    const details = [
    ...
    return {
        subject: `Lasclottes checkout started ${booking.public_reference}: ${dateOnly(booking.arrival)} to ${dateOnly(booking.departure)}`,
        html: `<div style="font-family:Arial,sans-serif;color:#243127;max-width:620px;margin:auto">
            <h1 style="font-family:Georgia,serif">A guest started Lasclottes checkout</h1>
            <p><strong>No payment has been confirmed yet.</strong> A separate paid-booking email will follow if Stripe confirms payment.</p>
            ${table(details)}
            <p>If no paid-booking email arrives, review Stripe and the booking operations log before contacting the guest.</p>
        </div>`,
        text: `A guest started Lasclottes checkout\n\nNo payment has been confirmed yet. A separate paid-booking email will follow if Stripe confirms payment.\n\n${details.map(([label, value]) => `${label}: ${value}`).join('\n')}\n\nIf no paid-booking email arrives, review Stripe and the booking operations log before contacting the guest.`
    };
};
...
const emailIdempotencyKey = (kind, bookingId, generation = 1) =>
    `${kind}/${bookingId}/v${Math.max(Number(generation) || 1, 1)}`;
...
const resendRequestOptions = ({ apiKey, idempotencyKey, body }) => ({
    method: 'POST',
    headers: resendHeaders(apiKey, idempotencyKey),
    body: JSON.stringify(body),
    signal: typeof AbortSignal !== 'undefined' && typeof AbortSignal.timeout === 'function'
        ? AbortSignal.timeout(RESEND_REQUEST_TIMEOUT_MS)
        : undefined
});

const sendEmailOnce = async ({ booking, kind, apiKey, from, to, replyTo, content }) => {
    const claim = await claimEmailDelivery(booking.id, kind);
    if (!claim) return { skipped: true, reason: 'already_sent' };
    try {
        const response = await fetch('https://api.resend.com/emails', resendRequestOptions({
            apiKey,
            idempotencyKey: emailIdempotencyKey(kind, booking.id, claim.send_generation),
            body: resendBody({ booking, kind, from, to, replyTo, content })
        }));
...
const emailConfiguration = (booking) => {
    ...
    const configuredMonitors = config.monitorEmails();
    const monitorEmails = [...new Set(configuredMonitors.map((value) => String(value).trim().toLowerCase()))]
        .filter((value) => value !== ownerEmail);
    if (!apiKey || !senderEmail || !validEmail(booking.email) || !validEmail(ownerEmail)) {
        throw new Error('Booking email is not configured.');
    }
    if (monitorEmails.some((value) => !validEmail(value))) {
        throw new Error('Booking monitor email is not configured.');
    }

    return { apiKey, from: `Lasclottes <${senderEmail}>`, ownerEmail, monitorEmails };
};

const sendBookingEmailKindOnce = async (booking, kind) => {
    if (!EMAIL_DELIVERY_KINDS.has(kind)) throw new Error('Invalid booking email kind.');
    const settings = emailConfiguration(booking);
    if (settings.skipped) return settings;
    const { apiKey, from, ownerEmail, monitorEmails } = settings;
    if (kind === 'guest_payment_confirmation') {
    ...
    const isMonitor = kind.startsWith('monitor_');
    const isCheckoutStarted = kind.endsWith('_checkout_started');
    if (isMonitor && monitorEmails.length === 0) {
        return { skipped: true, reason: 'monitor_not_configured' };
    }
    return sendEmailOnce({
        booking,
        kind,
        apiKey,
        from,
        to: isMonitor ? monitorEmails : ownerEmail,
        replyTo: booking.email,
        content: isCheckoutStarted ? buildCheckoutStartedEmail(booking) : buildOwnerEmail(booking)
    });
};

const sendKindsTogether = async (booking, kinds) => {
    const settled = await Promise.allSettled(kinds.map((kind) => sendBookingEmailKindOnce(booking, kind)));
    const results = {};
    const failures = [];
    settled.forEach((result, index) => {
        const kind = kinds[index];
        if (result.status === 'fulfilled') results[kind] = result.value;
        else failures.push(`${kind}: ${result.reason?.message || 'delivery failed'}`);
    });
    if (failures.length) throw new Error(`Booking notifications failed (${failures.join('; ')}).`);
    return results;
};

const sendBookingConfirmationOnce = async (booking) => {
    if (!config.emailsEnabled()) return { skipped: true, reason: 'emails_disabled' };
    const results = await sendKindsTogether(booking, [
        'guest_payment_confirmation',
        'owner_booking_notification',
        'monitor_booking_notification'
    ]);
    return {
        guest: results.guest_payment_confirmation,
        owner: results.owner_booking_notification,
        monitor: results.monitor_booking_notification
    };
};

const sendCheckoutStartedNotifications = async (booking) => {
    if (!config.emailsEnabled()) return { skipped: true, reason: 'emails_disabled' };
    const results = await sendKindsTogether(booking, [
        'owner_checkout_started',
        'monitor_checkout_started'
    ...
    return {
        owner: results.owner_checkout_started,
        monitor: results.monitor_checkout_started
    };
};

const retryBookingEmailDelivery = async (publicReference, kind) => {
    const reference = String(publicReference || '').trim().toUpperCase();
    if (!/^LC-[A-Z0-9]{6,20}$/.test(reference) || !EMAIL_DELIVERY_KINDS.has(kind)) {
        throw new Error('Invalid email retry request.');
    }
    const booking = await prepareEmailDeliveryRetry(reference, kind);
    if (!booking) return null;
    return {
        reference: booking.public_reference,
        kind,
        delivery: await sendBookingEmailKindOnce(booking, kind)
    };
};
...
module.exports = {
    ...
    senderAddress,
    retryBookingEmailDelivery,
    sendBookingEmailKindOnce,
    sendBookingConfirmationOnce,
    sendCheckoutStartedNotifications,
    validDomain
};
