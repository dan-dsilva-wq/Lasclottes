...
const emailConfiguration = (booking) => {
    ...
    return { apiKey, from: `Lasclottes <${senderEmail}>`, ownerEmail };
};
...
const sendBookingEmailKindOnce = async (booking, kind) => {
    ...
    const { apiKey, from, ownerEmail } = settings;
...
const sendBookingConfirmationOnce = async (booking) => {
    ...
    const guest = await sendBookingEmailKindOnce(booking, 'guest_payment_confirmation');
    const owner = await sendBookingEmailKindOnce(booking, 'owner_booking_notification');
    return { guest, owner };
};
