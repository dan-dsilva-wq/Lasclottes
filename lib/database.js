...
const runSchema = async () => {
    ...
    await sql`ALTER TABLE bookings ADD COLUMN IF NOT EXISTS agreement_version text`;
    await sql`ALTER TABLE bookings ADD COLUMN IF NOT EXISTS agreement_accepted_at timestamptz`;
    await sql`ALTER TABLE bookings ADD COLUMN IF NOT EXISTS agreement_snapshot jsonb`;
    await sql`
        DO $$
        BEGIN
            IF NOT EXISTS (
    ...
    await sql`ALTER TABLE email_deliveries ADD COLUMN IF NOT EXISTS send_generation integer NOT NULL DEFAULT 1`;
    await sql`
...
const expireOldHolds = async () => {
    ...
    await sql`
        ...
        WHERE status IN ('creating_checkout', 'pending_payment', 'testing_checkout', 'testing_payment')
    ...
    try {
        const rows = await sql`
            INSERT INTO bookings (
                ...
                damage_deposit_pence, tourist_tax_eur_cents,
                agreement_version, agreement_accepted_at, agreement_snapshot,
                hold_expires_at
            ) VALUES (
                ${publicReference}, ${idempotencyKey}, ${allowOverlap ? 'testing_checkout' : 'creating_checkout'},
                ...
                ${Number.isFinite(quote.touristTaxEur) ? toMinorUnits(quote.touristTaxEur) : 0},
                ${agreementVersion}, now(), CAST(${JSON.stringify(agreementSnapshot)} AS jsonb),
                now() + (${holdMinutes} * interval '1 minute')
            )
            RETURNING *
        `;
...
const markCheckoutPending = async (bookingId, sessionId) => {
    ...
    const rows = await sql`
        UPDATE bookings
        SET status = CASE
                WHEN status = 'testing_checkout' THEN 'testing_payment'
                ELSE 'pending_payment'
            END,
            stripe_checkout_session_id = ${sessionId}, updated_at = now()
        WHERE id = ${bookingId}
        RETURNING *
    `;
    return rows[0] || null;
...
const cancelBooking = async (bookingId, status = 'checkout_failed') => {
    ...
    await sql`
        ...
        WHERE id = ${bookingId} AND status NOT IN ('paid', 'test_paid')
    `;
};
...
const markBookingPaid = async ({ bookingId, sessionId, paymentIntentId, amountTotal, currency }) => {
    ...
    const rows = await sql`
        UPDATE bookings
        SET status = CASE
                WHEN status IN ('testing_checkout', 'testing_payment', 'test_paid') THEN 'test_paid'
                ELSE 'paid'
            END,
...
const listEmailDeliveryIssues = async (limit = 50) => {
    await ensureSchema();
    const sql = await getSql();
    ...
    return sql`
        SELECT booking.public_reference,
               ...
               booking.email,
               booking.phone,
               booking.arrival::text AS arrival,
               booking.departure::text AS departure,
               delivery.kind,
               delivery.status,
               delivery.attempts,
        ...
        WHERE booking.status IN ('paid', 'test_paid')
          AND (
              delivery.status = 'failed'
              OR delivery.provider_status IN ('bounced', 'complained', 'failed', 'suppressed')
          )
...
const prepareEmailDeliveryRetry = async (publicReference, kind, maxAttempts = 6) => {
    await ensureSchema();
    const sql = await getSql();
    const rows = await sql`
        UPDATE email_deliveries AS delivery
        SET status = 'failed',
            send_generation = delivery.send_generation + CASE
                WHEN delivery.provider_status IN ('bounced', 'complained', 'failed', 'suppressed') THEN 1
                ELSE 0
            END,
            provider_id = CASE
                WHEN delivery.provider_status IN ('bounced', 'complained', 'failed', 'suppressed') THEN NULL
                ELSE delivery.provider_id
            END,
            provider_status = NULL,
            provider_status_detail = NULL,
            last_provider_event_at = NULL,
            last_error = 'Owner requested an email retry.',
            updated_at = now()
        FROM bookings AS booking
        WHERE booking.id = delivery.booking_id
          AND booking.public_reference = ${publicReference}
          AND booking.status IN ('paid', 'test_paid')
