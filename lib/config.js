...
const STAGING_HOSTNAME = 'lasclottes.super-bread-8b96.workers.dev';
const STAGING_ORIGIN = `https://${STAGING_HOSTNAME}`;
const isStagingOrigin = (origin) => String(origin || '').replace(/\/$/, '') === STAGING_ORIGIN;
const isStagingHostname = (hostname) => String(hostname || '')
    .trim()
    .toLowerCase()
    .split(':')[0] === STAGING_HOSTNAME;
