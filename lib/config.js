...
const listValue = (...names) => firstValue(...names)
    .split(',')
...
const isStagingHostname = (hostname) => TEST_HOSTNAMES.has(String(hostname || '')
    .trim()
    .toLowerCase()
    .split(':')[0]);
const isStagingOrigin = (origin) => {
    try {
        const parsed = new URL(String(origin || ''));
        return parsed.protocol === 'https:' && isStagingHostname(parsed.hostname);
    } catch (_) {
        return false;
    }
};
...
const config = {
    ...
    stripeLiveSecretKey: () => firstValue('STRIPE_LIVE_SECRET_KEY', 'STRIPE_SECRET_KEY'),
    stripeWebhookSecret: () => firstValue('TEST_STRIPE_WEBHOOK_SECRET', 'STRIPE_WEBHOOK_SECRET'),
    stripeLiveWebhookSecret: () => firstValue('STRIPE_LIVE_WEBHOOK_SECRET', 'STRIPE_WEBHOOK_SECRET'),
    ...
    monitorEmails: () => listValue('BOOKING_MONITOR_EMAILS'),
    operationsToken: () => firstValue('BOOKING_OPERATIONS_TOKEN'),
    wixBridgeToken: () => firstValue('WIX_BOOKING_BRIDGE_TOKEN'),
    wixSiteBaseUrls: () => listValue('WIX_SITE_BASE_URLS'),
    wixStripeSuccessUrl: () => firstValue('WIX_STRIPE_SUCCESS_URL'),
    wixStripeCancelUrl: () => firstValue('WIX_STRIPE_CANCEL_URL'),
    wixTestMode: () => isEnabled('WIX_BOOKING_TEST_MODE'),
    wixLivePayments: () => isEnabled('WIX_LIVE_PAYMENTS'),
    liveStripeWebhooksOnSharedEndpoint: () => isEnabled('STRIPE_LIVE_WEBHOOKS_ON_SHARED_ENDPOINT'),
