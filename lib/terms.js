...
const BOOKING_TERMS_VERSION = '2026-08-26-draft-6';
...
const BOOKING_TERMS_TEXT = [
    ...
    'Price and payment: The accommodation price is shown in pounds sterling (GBP). If arrival is more than 60 days away, an initial 20% booking payment is due on confirmation. That initial payment is arrhes: if the guest withdraws, it is normally retained by Lasclottes; if Lasclottes withdraws, Lasclottes normally repays twice that amount. The remaining accommodation balance and the refundable GBP 500 damage deposit are due 60 days before arrival. When booking within 60 days of arrival, the full accommodation price and damage deposit are due on confirmation.',
    'Tourist tax: The owner confirms that the accommodation has a three-star classification valid until 3 October 2028. Tourist tax is not included in the GBP card figures; the official rate for the guest\'s year of arrival applies. Lasclottes will confirm the amount in euros, the people to whom it applies and the payment method before the balance is due.',
    'Cancellation: If the guest withdraws more than 60 days before arrival, the 20% arrhes are normally retained and no further accommodation payment is due. The owner-approved treatment of the remaining accommodation balance following a cancellation, curtailment or failure to arrive within 60 days has not yet been inserted in this draft. Suitable travel insurance is strongly recommended.',
...
const bookingAgreementSnapshot = ({ quote, publicReference }) => ({
    ...
    price: {
        ...
        touristTaxMinorUnits: Number.isFinite(quote.touristTaxEur)
            ? Math.round(quote.touristTaxEur * 100)
            : null
    },
